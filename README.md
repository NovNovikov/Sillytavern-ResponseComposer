...
**Use Regex and QR** is the master switch for both behaviours below. When it is enabled, **Apply SillyTavern Regex and before-generation Quick Replies** on a Generate block runs active Quick Replies marked **Execute before message generation** before that block's condition is evaluated. This lets a QR set a mode variable which the same block can test through **Tavern variable matches pattern**. It uses Tavern's standard Quick Reply lifecycle, so global, chat and character Quick Reply sets follow their ordinary enablement and recursion safeguards.
